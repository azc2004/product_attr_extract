...
def main():
    ...
    if search_results:
        ...
        with st.expander("프롬프트 수정하기 (클릭하여 열기/닫기)", expanded=True):
            ...
            # 메인 화면에 꽉 차게(height=300) 배치
            user_system_prompt = st.text_area(
                label="시스템 프롬프트 내용",
                value=DEFAULT_SYSTEM_PROMPT,
                height=300, # 높이를 충분히 주어 길게 작성 가능하게 함
                key="system_prompt_input", # 키를 지정하여 상태 유지
                help="AI의 페르소나와 분석 규칙을 정의합니다."
            )

            # ★ [핵심] UI에서 받은 프롬프트가 있으면 그걸 쓰고, 없으면 기본값 사용
            final_prompt = user_system_prompt if user_system_prompt else DEFAULT_SYSTEM_PROMPT
